	...
	private void sysInit() {
		...
		try {
			...
			if (rsp.getStatus() == Status.NOT_FOUND.getStatusCode()) {
				...
				// // upload ondemand price
				// URL ondemandUrl = MetadataLoader.class.getClassLoader().getResource("init-data/ondemand.json");
				// JsonNode ondemand = (JsonNode) mapper.readTree(ondemandUrl);
				// CostTypeEnum costType = InstancePrice.CostTypeEnum.EV_ondemand;
				// ArrayNode regions = (ArrayNode) ondemand.get("regions");
				// for (int i = 0; i < regions.size(); i++) {
				// JsonNode regionNode = regions.get(i);
				// String region = regionNode.get("region").asText();
				// ArrayNode instanceTypes = (ArrayNode) regionNode.get("instanceTypes");
				// for (int j = 0; j < instanceTypes.size(); j++) {
				// JsonNode instanceType = instanceTypes.get(j);
				// String osType = instanceType.get("os").asText();
				// double price = instanceType.get("price").asDouble();
				// String type = instanceType.get("type").asText();
				// InstancePrice instancePrice = new InstancePrice();
				// instancePrice.setCostType(costType);
				// instancePrice.setCloudName(Cloud.AWS.toString());
				// instancePrice.setRegionName(region);
				// instancePrice.setInstanceOs(osType);
				// instancePrice.setInstanceType(type);
				// instancePrice.setRate(price);
				// resource = client.resource(this.cmsUrl + "/repositories/ico/branches/main/InstancePrice");
				// resource.accept(MediaType.APPLICATION_JSON_TYPE)
				// .entity(instancePrice.toString(), MediaType.APPLICATION_JSON_TYPE).post();
				// }
				// }
				//
				// // upload reserved price
				// costType = InstancePrice.CostTypeEnum.EV_reserved;
				// regions = (ArrayNode) ondemand.get("regions");
				// for (int i = 0; i < regions.size(); i++) {
				// JsonNode regionNode = regions.get(i);
				// String region = regionNode.get("region").asText();
				// ArrayNode instanceTypes = (ArrayNode) regionNode.get("instanceTypes");
				...
				// JsonNode instanceType = instanceTypes.get(j);
				// String osType = instanceType.get("os").asText();
				// ReservationUtilizationEnum uenum = ReservationUtilizationEnum.fromString(instanceType.get(
				// "utilization").asText());
				// String type = instanceType.get("type").asText();
				// JsonNode prices = instanceType.get("prices");
				// for (int p = 0; p < 2; p++) {
				// String term = "";
				// ReserveDurationEnum termEnum = null;
				// if (p == 0) {
				// termEnum = InstancePrice.ReserveDurationEnum.EV_1year;
				// term = "1year";
				// } else {
				// termEnum = InstancePrice.ReserveDurationEnum.EV_3year;
				// term = "3year";
				//
				// }
				// JsonNode termPrice = prices.get(term);
				// if (termPrice.get("hourly") != null) {
				// double price = termPrice.get("hourly").asDouble();
				// double upfront = termPrice.get("upfront").asDouble();
				// InstancePrice instancePrice = new InstancePrice();
				// instancePrice.setCostType(costType);
				// instancePrice.setCloudName(Cloud.AWS.toString());
				// instancePrice.setRegionName(region);
				...
				// instancePrice.setInstanceType(type);
				// instancePrice.setRate(price);
				// instancePrice.setUpfrontFee(upfront);
				// instancePrice.setReserveDuration(termEnum);
				// instancePrice.setReservationUtilization(uenum);
				// resource = client.resource(this.cmsUrl
				// + "/repositories/ico/branches/main/InstancePrice");
				// resource.accept(MediaType.APPLICATION_JSON_TYPE)
				// .entity(instancePrice.toString(), MediaType.APPLICATION_JSON_TYPE).post();
				// }
				// }
