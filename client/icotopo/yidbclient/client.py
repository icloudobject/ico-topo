...
class YidbClient ():
    ...
    def post_change(self, url, payload, query_string = None):
        ...
        if query_string:
            return requests.post(url,data=json.dumps(payload), headers=headers, query_string=query_string)
        else:
    ...
    def post_topo_resource(self, repo, class_name, payload, client_id):
        "upsert a object into repo"
